...
int main (int argc, char *argv[])
{
  ...
  // Install frame capture model on all WifiPhy objects
  NetDeviceContainer allDevs;
  allDevs.Add (apDev);
  allDevs.Add (staDevs);
  for (uint32_t i = 0; i < allDevs.GetN (); i++)
    {
      Ptr<SimpleFrameCaptureModel> model = CreateObject<SimpleFrameCaptureModel> ();
      model->SetAttribute ("CaptureWindow", TimeValue (MicroSeconds (16))); 
      model->SetAttribute ("Margin", DoubleValue (5));
      Ptr<WifiNetDevice> wifiNetDevice = allDevs.Get (i)->GetObject<WifiNetDevice> ();
      Ptr<WifiPhy> wifiPhy = wifiNetDevice->GetPhy ();
      wifiPhy->SetFrameCaptureModel (model);
    }
